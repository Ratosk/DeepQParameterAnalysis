...
def GenerateOverallGraphic( folder = 'GamaResults', Legend = True, ShowPlot = False):
        ...
        for gamma in gammas:
            ...
            filteredfiles += [ file.split(".St")[0].split('_')[0:8] +[gamma] for file in files if file.split(".")[-1] == "Stats"]
        ...
        for file in filteredfiles:
            ...
            if not SecondKey in NetDictionary.keys():
                NetDictionary[SecondKey] = []
                MaxSpeed[file[1]][SecondKey] = 0

                
            Speed = float(file[0])
            
            if Speed > MaxSpeed[file[1]][SecondKey]:
                MaxSpeed[file[1]][SecondKey] = Speed
            
            if Speed > 45 and int(file[5]) > 100000: 
                NetDictionary[SecondKey].append( Speed )
        ...
        for Topology in OverallResults:
            ...
            pl.xlabel('Número de Agentes - Número de muestras analizadas por categoría: '+str( Longest))
            pl.ylabel('Velocidad Km/h')
            
            pl.grid(True)
            pl.savefig('ResAgents_'+Topology+'.png')
            
            pl.close()
            AllData.append(TempData)

        PrintLatexTable(AllData,OverallLabels,'Agents', gammas)
        NetSpeeds = {}
        for gamma in Labels:
            NetSpeeds[gamma] = []
        for elem in AllData:
        ...
        pl.xlabel('Número de Agentes - Número de muestras analizadas por categoría: '+str(OverallLen))
        pl.ylabel('Velocidad Km/h')
        pl.grid(True)
        ...
        pl.savefig('ResAgentsOverall.png')
        ...
        pl.plot(ResultStatistics, style=['-', '--', '-.'])
        ...
        for category in Labels:
            ...
            pl.annotate(str(Temp),
                xy=(Xcoord, Temp+0.2), xycoords='data')
        ...
        pl.ylabel('Velocidad Km/h')
        pl.grid(True)
        pl.savefig('StatAgentsOverall.png')
