...
class SimulationHandler():
    ...
    def SetExperimentParameters(self, NetName, NetInputType , learning_rate=0.001, batch_size=64, iterations=10000, otherAgents = 0, experience_size = 10000, temporalWindow = 0):
    ...
    def SaveStatistics(self): 
        self.FileName = str(self.AvgSpeed)+'_'+self.NetName + "_" + self.NetInputType + "_" + self.timeString + "_" + str(self.iterations) + "_" + str(self.experience_size)+ "_A" + str(self.otherAgents)
...
def Experiment(Net,InputType,semaphore,Agents):
    
        Simulation = SimulationHandler(semaphore = semaphore)
                
        Simulation.SetExperimentParameters(Net,InputType,iterations=200000, otherAgents = Agents)       
        Simulation.LoadCode()
...
for index in range(Trainings):
    ...
    for Agents in range(5,10,1):
        
        print("Starting training with",Agents,"additional agents.")
        ...
        for Net in NetTopologyDictionary:
            ...
            for InputType in NetInputTypes:
                ...
                t = threading.Thread(target=Experiment, name=Net+'_'+InputType, args = (Net,InputType,semaphore,Agents))
                SimThreads.append(t)
                t.setDaemon(True)
                t.start()
