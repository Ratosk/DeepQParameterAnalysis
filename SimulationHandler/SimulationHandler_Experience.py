...
class SimulationHandler():
    ...
    def __init__(self, OpenWebInstance = True, semaphore = None):
        '''
        Constructor
        '''
        ...
        if not self.Semaphore:
            print("WARNING> A semaphore was not provided, this experiment should not used in multithreading")
        else:
            if self.Semaphore.acquire(120):
                print("Semaphore acquired correctly, the thread can continue")
            else:
                print("WARNING> Semaphore timeout was expired this execution could be not syncronized")
    ...
    def SaveStatistics(self): 
        self.FileName = str(self.AvgSpeed)+'_'+self.NetName + "_" + self.NetInputType + "_" + self.timeString + "_" + str(self.iterations) + "_" + str(self.experience_size)
    ...
    def FinishExperiment(self):
        ...
        if self.Semaphore.acquire(120):
            print("Semaphore acquired correctly, the thread can continue",self.NetInputType)
        else:
            print("WARNING> Semaphore timeout was expired this execution could be not syncronized")
        ...
        if self.Semaphore: 
            ...
            print("Semaphore released!",self.NetInputType)
            
        timeout = 20
            
        while not os.path.exists(self.WorkSpacePath+self.FileName + ".netInfo.js"):
            sleep(5)
            timeout -= 5
            if not timeout:
                break
...
def Experiment(Net,InputType,semaphore):
    
        Simulation = SimulationHandler(semaphore = semaphore)
                
        Simulation.SetExperimentParameters(Net,InputType,iterations=200000,experience_size = experience)       
        Simulation.LoadCode()
        
        Simulation.RunExperiment()
        Simulation.FinishExperiment()
...
for index in range(Trainings):
    ...
    for experience in range(500,10500,500):
        
        print("Starting training with Experience size of:",experience)
        ...
        for Net in NetTopologyDictionary:
            ...
            for InputType in Sim.NetInputTypes:
                ...
                '''
                Simulations[Net][InputType] = SimulationHandler(semaphore = semaphore)
                
                Simulations[Net][InputType].SetExperimentParameters(Net,InputType,iterations=10000,
                                                                    experience_size = experience)
            ...
            for InputType in NetInputTypes:
                ...
                while(len(SimThreads)>4):
                    ...
                    if not thread.isAlive():
                            SimThreads.remove(thread)  
                '''
                t = threading.Thread(target=Experiment, name=Net+'_'+InputType, args = (Net,InputType,semaphore))
                SimThreads.append(t)
                t.setDaemon(True)
                t.start()
