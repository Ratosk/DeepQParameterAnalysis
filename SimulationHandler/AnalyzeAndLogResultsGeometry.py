...
def GenerateOverallGraphic( folder = 'TopologyInput', Legend = True, ShowPlot = False):
        ...
        for Topology in OverallResults:
            ...
            pl.grid(True)
            pl.savefig('ResTop_'+Topology+'.png')
            pl.close()
            PrintLatexTable(TempData,Labels,'Geo_'+Topology)
        ...
        Longest = 1000
        TempData = []
        Labels = ['Piramidal Invertida', 'Piramidal', 'Cardinal','Cuadrantes']
        for category in Labels:
        ...
        pl.legend(['Promedio', 'Mediana', 'Maximo'],
           shadow=True, loc=(0.70, 0.05), fontsize=12)
        index = 0 
        for category in Labels:
            Xcoord = index -0.1
